...
// @downloadURL https://update.sleazyfork.org/scripts/25781/JAV%E8%80%81%E5%8F%B8%E6%9C%BA.user.js
// @updateURL https://update.sleazyfork.org/scripts/25781/JAV%E8%80%81%E5%8F%B8%E6%9C%BA.meta.js
// ==/UserScript==
/* jshint -W097 */
(function () {
    'use strict';
 ...
 */
    class Common {
        ...
        static openSystemConfig() {
            ...
            let dom = `<div>
                       ...
                       <label class="tm-setting">馒头PT网址<input type="text" id="mtpt_url" class="tm-text" value="${GM_getValue('mtpt_url')}"></label>
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    GM_setValue('mtpt_url', $('#mtpt_url').val());
                    ...
                    $div_video_cast.after(`
                        <div id="video_webplayer" class="item">
                            <table><tbody><tr>
                                ...
                                <td>
                                    <span class="genre"><a href="https://njav.tv/zh/v/${AVID}" target="_blank" title="需解封印" >njav</a></span>
    ...
    var thirdparty = {
        ...
        nong: {
            ...
            resource_sites: {
                [GM_getValue('mtpt_url')]: (kw, cb) => {
                  let prom = new Promise((resolve, reject) => {
                      GM_xmlhttpRequest({
                          method: "POST",
                          url: `https://${GM_getValue('mtpt_url')}/api/torrent/search`,
                          headers: {
                              'Content-Type': 'application/json',
                              'x-api-key': 'c7fabb70-2767-4e8f-9845-504cbf045830',
                          },
                          dataType: "json",
                          data: JSON.stringify({
                            categories: [],
                          ...
                          timeout: 20000,
                          onload: (r) => resolve(r),
                          onerror: (r) => reject(`查找馒头PT的 ${kw} 出错了`),
                          ontimeout: (r) => reject(`查找馒头PT的 ${kw} 超时了`)
                      });
                  });
                  prom.then((result) => {
                    ...
                    let data = [];
                    if (resjson.code == "0" && resjson.data.data.length > 0) {
                        data = resjson.data.data.map(v => ({
                            "title": v.name,
                            "maglink": `https://kp.m-team.cc/detail/${v.status.id}`,
                            "size": `${(parseInt(v.size) / 1024**3).toFixed(2)}GB`,
                            "src": `https://kp.m-team.cc/detail/${v.status.id}`,
                        }))
                    } else {
                        data.push({
                                  "title": "没有找到磁链接",
